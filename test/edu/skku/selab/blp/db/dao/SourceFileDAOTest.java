...
public class SourceFileDAOTest {
	...
	private void prepareTestingData() throws Exception {
		...
		assertNotEquals("fileNameWithPath1 insertion failed!", BaseDAO.INVALID,
				sourceFileDAO.insertSourceFile(fileNameWithPath1, className1));
		assertNotEquals("fileNameWithPath2 insertion failed!", BaseDAO.INVALID,
				sourceFileDAO.insertSourceFile(fileNameWithPath2, className2));

		HashMap<String, Integer> fileInfo = sourceFileDAO.getSourceFiles();
		...
		sourceFileID = sourceFileDAO.getSourceFileID(fileNameWithPath1);
		...
		sourceFileID = sourceFileDAO.getSourceFileID(fileNameWithPath2);
		assertNotEquals("fileNameWithPath2's corpusSet insertion failed!", BaseDAO.INVALID,
				sourceFileDAO.insertCorpusSet(sourceFileID, version2, corpus3, totalCorpusCount3, lengthScore3));		
		
		assertEquals("Source file count is WRONG!", 2, sourceFileDAO.getSourceFileCount(version1));
		assertEquals("Source file count is WRONG!", 3, sourceFileDAO.getSourceFileCount(version2));
		
		HashMap<String, String> classNames = sourceFileDAO.getClassNames(version2);
		...
		HashMap<String, SourceFileCorpus> corpusMap = sourceFileDAO.getCorpusMap(version1);
		...
		HashMap<String, Double> lengthScores = sourceFileDAO.getLengthScores(version1);
		...
		assertNotEquals("term1 insertion failed!", BaseDAO.INVALID, sourceFileDAO.insertTerm(term1));
		assertNotEquals("term2 insertion failed!", BaseDAO.INVALID, sourceFileDAO.insertTerm(term2));
		
		HashMap<String, Integer> wordMap = sourceFileDAO.getTermMap();
	...
	public void verifyGetSourceFileAnalysisValue() throws Exception {
		...
		double tf = 0.23;
		double idf = 0.42;
		AnalysisValue termWeight1 = new AnalysisValue(fileName1, version1, term1, termCount, idvDocCount, tf, idf);
		assertNotEquals("analysisValue1 insertion failed!", BaseDAO.INVALID, sourceFileDAO.insertTermWeight(termWeight1));
		
		AnalysisValue returnValue = sourceFileDAO.getTermWeight(fileName1, version1, term1);
	...
	public void verifyGetImportedClasses() throws Exception {
		...
		sourceFileVersionID = sourceFileDAO.getSourceFileVersionID(fileName2, version1);
		assertNotEquals("importedClass insertion failed!", BaseDAO.INVALID, sourceFileDAO.insertImportedClasses(sourceFileVersionID, importedClasses));
		
		HashMap<String, ArrayList<String>> importedClassesMap = sourceFileDAO.getAllImportedClasses(version1);
