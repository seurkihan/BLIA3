...
public class DbUtil {
	...
	public int createAllAnalysisTables() throws Exception {
		String sql = "CREATE MEMORY TABLE SF_INFO(SF_ID INT PRIMARY KEY HASH AUTO_INCREMENT, SF_NAME VARCHAR(255), CLS_NAME VARCHAR(255)); " +
				...
				"CREATE MEMORY TABLE SF_VER_INFO (SF_VER_ID INT PRIMARY KEY HASH AUTO_INCREMENT, SF_ID INT, VER VARCHAR(15),"
				+ " COR VARCHAR, CLS_COR VARCHAR, MTH_COR VARCHAR, VAR_COR VARCHAR, CMT_COR VARCHAR, API_COR VARCHAR, TOT_CNT INT, LEN_SCORE DOUBLE,"
				+ " COR_NORM DOUBLE, CLS_COR_NORM DOUBLE, MTH_COR_NORM DOUBLE, VAR_COR_NORM DOUBLE, CMT_COR_NORM DOUBLE, API_COR_NORM DOUBLE); " +
				"CREATE INDEX COMP_IDX_SF_VER_ID ON SF_VER_INFO(SF_ID, VER); " +
				
				"CREATE MEMORY TABLE SF_TERM_INFO (SF_TERM_ID INT PRIMARY KEY HASH AUTO_INCREMENT, TERM VARCHAR(255)); " + 
				"CREATE UNIQUE HASH INDEX IDX_SF_TERM ON SF_TERM_INFO(TERM); " +
				
				"CREATE MEMORY TABLE SF_IMP_INFO (SF_VER_ID INT, IMP_CLASS VARCHAR(255)); " +
				"CREATE INDEX IDX_SF_IMP_INFO ON SF_IMP_INFO(SF_VER_ID); " +
				
				"CREATE MEMORY TABLE SF_TERM_WGT (SF_VER_ID INT, SF_TERM_ID INT, TERM_CNT INT, INV_DOC_CNT INT, TF DOUBLE, IDF DOUBLE); " +
				"CREATE UNIQUE INDEX COMP_IDX_SF_TERM_WGT ON SF_TERM_WGT(SF_VER_ID, SF_TERM_ID); " +
				
				"CREATE MEMORY TABLE MTH_INFO(MTH_ID INT PRIMARY KEY HASH AUTO_INCREMENT, SF_VER_ID INT, MTH_NAME VARCHAR(127), RET_TYPE VARCHAR(63), PARAMS VARCHAR(255), HASH_KEY VARCHAR(40)); " +
				"CREATE HASH INDEX IDX_MTH_INFO_SF_VER_ID ON MTH_INFO(SF_VER_ID); " +
				...
				+ " COR VARCHAR, SMR_COR VARCHAR, DESC_COR VARCHAR, CMT_COR VARCHAR, TOT_CNT INT,"
				+ " COR_NORM DOUBLE, SMR_COR_NORM DOUBLE, DESC_COR_NORM DOUBLE, MTH_TOT_CNT INT, MTH_NORM DOUBLE, VER VARCHAR(15)); " + 
				...
				"CREATE MEMORY TABLE BUG_STRACE_INFO (BUG_ID INT, STRACE_CLASS VARCHAR(255)); " +
				"CREATE INDEX IDX_BUG_STRACE_INFO ON BUG_STRACE_INFO(BUG_ID); " +
				
				"CREATE MEMORY TABLE BUG_SF_TERM_WGT(BUG_ID INT, SF_TERM_ID INT, TERM_CNT INT, INV_DOC_CNT INT, TF DOUBLE, IDF DOUBLE); " +
				"CREATE UNIQUE INDEX COMP_IDX_BUG_SF_TERM_WGT ON BUG_SF_TERM_WGT(BUG_ID, SF_TERM_ID); " +

				"CREATE MEMORY TABLE BUG_MTH_TERM_WGT(BUG_ID INT, MTH_TERM_ID INT, TERM_CNT INT, INV_DOC_CNT INT, TF DOUBLE, IDF DOUBLE); " +
				...
				"CREATE MEMORY TABLE BUG_FIX_SF_INFO(BUG_ID INT, FIXED_SF_VER_ID INT); " +
				"CREATE INDEX IDX_BUG_FIX_SF_INFO ON BUG_FIX_SF_INFO(BUG_ID); " +
				...
				"CREATE MEMORY TABLE VER_INFO(VER VARCHAR(15) PRIMARY KEY HASH, REL_DATE DATETIME); " +

				"CREATE MEMORY TABLE INT_ANALYSIS(BUG_ID INT, SF_VER_ID INT, VSM_SCORE DOUBLE, SIMI_SCORE DOUBLE, BL_SCORE DOUBLE, STRACE_SCORE DOUBLE, COMM_SCORE DOUBLE, MID_SF_SCORE DOUBLE, BLIA_SF_SCORE DOUBLE); " +
				"CREATE INDEX IDX_INT_ANAL_MID_SF_SCORE ON INT_ANALYSIS(MID_SF_SCORE DESC); " +
				"CREATE INDEX IDX_INT_ANAL_BLIA_SF_SCORE ON INT_ANALYSIS(BLIA_SF_SCORE DESC); " +
				...
				"CREATE MEMORY TABLE INT_MTH_ANALYSIS(BUG_ID INT, MTH_ID INT, VSM_SCORE DOUBLE, COMM_SCORE DOUBLE, SIMI_SCORE DOUBLE, BLIA_MTH_SCORE DOUBLE); " +
				"CREATE INDEX IDX_INT_MTH_ANAL_BLIA_MTH_SCORE ON INT_MTH_ANALYSIS(BLIA_MTH_SCORE DESC); " +
				"CREATE INDEX IDX_INT_MTH_ANAL_BUG_ID ON INT_MTH_ANALYSIS(BUG_ID); " + 
				"CREATE UNIQUE INDEX COMP_IDX_INT_MTH_ANALYSIS ON INT_MTH_ANALYSIS(BUG_ID, MTH_ID); ";
	...
	public int createEvaluationTable() throws Exception {
		String sql = "CREATE MEMORY TABLE EXP_INFO(TOP1 INT, TOP5 INT, TOP10 INT, TOP1_RATE DOUBLE, TOP5_RATE DOUBLE, TOP10_RATE DOUBLE, MAP DOUBLE, MRR DOUBLE, PROD_NAME VARCHAR(31),"
					+ " ALG_NAME VARCHAR(31), ALG_DESC VARCHAR(255), ALPHA DOUBLE, BETA DOUBLE, GAMMA DOUBLE, DELTA DOUBLE, PAST_DAYS INT, EXP_DATE DATETIME); " +
